...
let Score_Counter_Dislikes_DOM = document.querySelector(
...
function Update_Score_Counters() {
  ...
  Score_Counter_Likes_DOM.innerHTML = `${roundToNearestTenth(
    Like_Percentage
  )}%(${Current_Score.Like})`;
  Score_Counter_Dislikes_DOM.innerHTML = `${roundToNearestTenth(
    Dislike_Percentage
  )}%(${Current_Score.Dislike})`;
...
function Load_Total_Score() {
  ...
  let Dislike_Saved =
    parseInt(localStorage.getItem("Total_Score_Dislike")) || 0;
